...
pub fn compile_project() {
    ...
    for path in template_paths.iter() {
        ...
        match parser::file(tokens, Some(path.clone()), None, None) {
            ...
            Err(error) => eprintln!("{}", reporting::draw_error(&error.0.unpack(), &DrawingInfo::from((&error.1, Some(&path))))),
        }
    }
    ...
    for path in input_paths.iter() {
        ...
        let tokens = lexer::tokenize(&input_string);
        match parser::file(tokens, Some(path.clone()), Some(&main_template), Some(&template_files)) {
            Ok(mut x) => {
        ...
        match html::generate_html(&file, vec![], &settings) {
            Ok(x) => {
                ...
                if let Some(path) = &file.path {
                    let mut output_path = output_path.join::<PathBuf>(path.iter().skip(1).collect());
                    ...
                    match write!(file, "{}", file_text) {
                        Ok(x) => x,
                        Err(x) => {errors.push(Error::IOError(x, output_path.clone())); continue},
                    };
                }
            },
            Err(errors) => {
